...
const dataPath = path.join(sysRoot, '.mctlauncher')
...
exports.getMusicMuted = function(){
    return config.settings.launcher.audio.muted
}
...
exports.getMusicVolume = function(){
    return config.settings.launcher.audio.volume
}

exports.setMusicVolume = function(volume){
    const parsed = Number(volume)
    config.settings.launcher.audio.volume = Math.max(0, Math.min(1, Number.isFinite(parsed) ? parsed : 0.5))
}
